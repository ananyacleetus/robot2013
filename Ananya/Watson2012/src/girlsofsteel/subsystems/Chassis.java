...
public class Chassis extends Subsystem {
    ...
    private static final double TURNING_SCALE = .5;//turning is half the speed
    //of straight
...
//            RobotMap.ENCODER_LEFT_CHANNEL_B, true, CounterBase.EncodingType.k4X);    
    ...
    private final double ENCODER_UNIT_RIGHT = (WHEEL_DIAMETER * Math.PI * GEAR_RATIO) / PULSES_RIGHT; //m per s
    private final double ENCODER_UNIT_LEFT = (WHEEL_DIAMETER * Math.PI * GEAR_RATIO) / PULSES_LEFT;
    ...
    private Encoder leftEncoder = new Encoder(RobotMap.ENCODER_LEFT_CHANNEL_A,
            RobotMap.ENCODER_LEFT_CHANNEL_B, false, CounterBase.EncodingType.k4X);  
    private final double PRACTICE_GEAR_RATIO = 12.0 / 22.0;
    private final double ENCODER_PRACTICE_RIGHT = (WHEEL_DIAMETER * Math.PI * PRACTICE_GEAR_RATIO) / 360.0;
    private final double ENCODER_PRACTICE_LEFT = (WHEEL_DIAMETER * Math.PI * PRACTICE_GEAR_RATIO) / 250.0;
     
    //create info for PIDs
    
    //rate p & i & d values -> same for the left & right -> tuned for PIT comp
    private final static double rateP = 1.1;//0.75;
    private final static double rateI = 0.085;//0.1;
    private final static double rateD = 0.0;
    //position good for both the practice & real robot
    private final static double positionP = 0.4;
    private final static double positionI = 0.0;//for the bridge -> might need i term
    private final static double positionD = 0.00075;
    ...
    private EncoderGoSPIDController rightPositionPID = new EncoderGoSPIDController(
            positionP, positionI, positionD, rightEncoder,
            new PIDOutput() {
    ...
    private EncoderGoSPIDController leftPositionPID = new EncoderGoSPIDController(
            positionP, positionI, positionD, leftEncoder,
            new PIDOutput() {
    ...
    //make sure you know HOW to find it so it will be easy to find on the real Watson at comp
    ...
    public void setLeftJags(double speed) {
        leftJags.set(-speed);//motors need to run the same way as the right
    }
    ...
    public void driveJagsLinearSlowTurning(double xAxis,double yAxis){
        xAxis = deadzone(xAxis, DEADZONE_RANGE)*TURNING_SCALE;
    ...
    public void setPositionPIDValues(double p, double i, double d) {
        rightPositionPID.setPID(p,i,d);
        leftPositionPID.setPID(p,i,d);
    }
    
    public void setPIDsPosition(){
        rightPositionPID.setPID(positionP,positionI,positionD);
        leftPositionPID.setPID(positionP, positionI, positionD);
    }
    ...
    public void initEncoders() {
        rightEncoder.setDistancePerPulse(ENCODER_PRACTICE_RIGHT);
        leftEncoder.setDistancePerPulse(ENCODER_PRACTICE_LEFT);
