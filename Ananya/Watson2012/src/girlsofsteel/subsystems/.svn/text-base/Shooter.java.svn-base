...
public class Shooter extends Subsystem {
    ...
    public final double KEY_SPEED = 24.0;//for everything that uses this -> in case
    //it changes for some reason
    
    //button board stufficles
    public final double MIN_SLIDER = 0.0;
    public final double MAX_SLIDER = 3.15;
    
    public final double VELOCITY_ERROR_RANGE = 0.5;//this is the speed that the
    ...
    //WE NEED TO RETUNE THESE SO IT IS NICE & DOESN'T VARY WITHIN 2M/S -> goal: 0.2m/s
    double p = 0.15;//0.1;//0.25;
    double i = 0.06;//0.0002;
    double d = 0.0;
    ...
    public void setJags(double speed) {
        jags.set(-speed);//motors run backwards
    }
    ...
    public void topRollersForward() {
        topRollers.set(Relay.Value.kForward);
    }

    public void topRollersBackward() {
        topRollers.set(Relay.Value.kReverse);
    }

    public void topRollersOff() {
        topRollers.set(Relay.Value.kOff);
    }
