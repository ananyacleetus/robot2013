...
public class Turret extends Subsystem implements PIDOutput, PIDSource {
    ...
    public final double OFFSET_ANGLE = -2.0*(Math.PI/180.0); //TODO test & find the offset angle of the turret for shooting
    ...
    Encoder encoder = new Encoder(RobotMap.ENCODER_TURRET_CHANNEL_A,
            RobotMap.ENCODER_TURRET_CHANNEL_B, true, Encoder.EncodingType.k4X);
...
//        setDefaultCommand(new TurretTrackTarget());
    }
    ...
    public void setJagSpeed(double speed) {
        turretJag.set(-speed);//the motor moves backward -> encoder is negative
    }
    ...
    public double getTurretAngle() {
        ...
        angle = angle % 360;//sam as above -> must be a positive number
    ...
    public void autoTrack() { //only works when the camera has the target (a boolean
        ...
        double setPoint;
        setPoint = diffAngle / 4 + getTurretAngle() - OFFSET_ANGLE;
