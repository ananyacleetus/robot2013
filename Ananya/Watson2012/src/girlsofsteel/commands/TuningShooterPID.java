...
public class TuningShooterPID extends CommandBase {
    ...
    int meanLength = (int) ((maxP-pStep)/pStep);
    ...
    protected void execute(){
        for(double p=pStep; p<maxP; p+=pStep){//starts at pStep, goes to maxP - pStep
            for(int setPoint = startSetPoint; setPoint<=maxSetPoint; setPoint++){
                ...
                shooter.setPIDValues(p, 0.0, 0.0);
                ...
                System.out.println("P=" + p + " & Set Point=" + setPoint
            ...
            for(int i=0; i<meanLength; i++){
                if(deviationMeans[i] == 0){//find the next empty slot in the means
                    ...
                    number = i;
                    System.out.println("P=" + p + "Dev Mean:" + deviationMean + "="
