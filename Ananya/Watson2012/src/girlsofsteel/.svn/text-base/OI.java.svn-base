...
improt girlsofsteel.commands.TopRolllersFoward;

public class OI {

    private final double NORMAL_DRIVE = 1.0;
    private final double MEDIUM_DRIVE = 0.75;
    private final double SLOW_DRIVE = 0.5;
    ...
    private static final int DRIVE_SLOW_VELOCITY_BUTTON = 1; //start
    ...
    private static final int ENABLE_Top_Rollers_BUTTON = 8; //R2 on operatorJoystick
    private static JoystickButton enableTopRollers;
    ...
    public OI() {
        ...
        normalDriveJags.whenPressed(new DriveSlowTurning(NORMAL_DRIVE));
        slowDriveJags.whenPressed(new DriveSlowTurning(SLOW_DRIVE));
        mediumDriveJags.whenPressed((new DriveSlowTurning(MEDIUM_DRIVE)));
        driveSlowVelocity.whenPressed(new DriveSlowVelocity());
        bridgeArmDown.whenPressed(new BridgeDown());
        bridgeArmUp.whenPressed(new BridgeUp());
        enableTopRollers.whenPressed(new TopRollersFoward());
        //TODO ask drive team if they want bridge functions to be "while held" functions
...
//        }
    ...
    //switches
    //FOR TESTING WE ARE MAKING THIS PUBLIC CHANGE BACK 
